...
type DeleteProjectRow struct {
	...
	SiblingKey sql.NullString `json:"sibling_key"`
	Threads    sql.NullInt64  `json:"threads"`
}
...
func (q *Queries) DeleteProject(ctx context.Context, arg DeleteProjectParams) ([]DeleteProjectRow, error) {
	rows, err := q.db.QueryContext(ctx, deleteProject, arg.ProjectID, arg.TeamID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&i.Deleted, &i.SiblingKey, &i.Threads); err != nil {
			return nil, err
		}

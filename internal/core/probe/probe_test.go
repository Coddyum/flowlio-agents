...
func TestNilCacheIsSafe(t *testing.T) {
	...
	probe.RecordEvent(nil, team, 1)
	probe.RecordCursor(nil, token, 1)
	probe.Seed(nil, team, token, 1, 1)
	if _, ok := probe.Head(nil, team); ok {
...
func TestColdCacheReportsMissing(t *testing.T) {
	c := newCache()
	if _, ok := probe.Head(c, uuid.New()); ok {
...
func TestRecordEventIsMonotonic(t *testing.T) {
	...
	probe.RecordEvent(c, team, 10)
	if h, _ := probe.Head(c, team); h != 10 {
		t.Fatalf("head = %d, want 10", h)
	}

	probe.RecordEvent(c, team, 5) // lower: ignored
	if h, _ := probe.Head(c, team); h != 10 {
		t.Errorf("head = %d after a lower bump, want 10 — the head went backwards", h)
	}

	probe.RecordEvent(c, team, 12) // higher: taken
	if h, _ := probe.Head(c, team); h != 12 {
...
func TestSeedWarmsBoth(t *testing.T) {
	c := newCache()
	team, tok := uuid.New(), uuid.New()

	probe.Seed(c, team, tok, 20, 20)

	h, hOK := probe.Head(c, team)
